...
int config_open(struct s_config *config, const char *path) {
    ...
    if (state ==0) {
    	...
    	if (fscanf(f,"%i\t%i\t%i\t%i\n",&config->throttle_standby,&config->throttle_min,&config->throttle_inflight,&config->throttle_midflight)!=4) state = 1;
        ...
        if (fscanf(f,"%i\t%i\n",&config->mpu_addr, &config->reset_gpio)!=2) state = 1;

        fclose(f);

        fflush(NULL);
	if (verbose) printf("AVRSPI: config read successfully\n");
    }
